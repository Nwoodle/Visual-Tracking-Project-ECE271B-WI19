...
pmu = sum(posx.feature.*posx.sampleImage.weight,2);
posmu = repmat(pmu,1,pcol);
sigm1 = sum((posx.feature-posmu).^2.*posx.sampleImage.weight,2);

nmu = sum(negx.feature.*negx.sampleImage.weight,2);
[nrow,ncol] = size(negx.feature);
negmu = repmat(nmu,1,ncol);
sigm0 = sum((negx.feature-negmu).^2.*negx.sampleImage.weight,2);
%------------------------------------------Our update method
%{
